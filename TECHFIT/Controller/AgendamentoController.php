...
class AgendamentoController {
    ...
    public function criar($alunoId, $professorId, $modalidade, $dataAula, $horario) {
        ...
        if (!in_array($modalidade, ['Boxe', 'Muay Thai'])) {
    ...
    public function listarAlunosPorTurma($modalidade, $dataAula, $horario) {
        return $this->dao->listarAlunosPorTurma($modalidade, $dataAula, $horario);
    }

    // ------------------ CANCELAR AGENDAMENTO (por aluno) - Remove participação do aluno ------------------
    public function cancelar($id, $alunoId) {
        ...
        // Salvar informações antes de deletar para usar na lista de espera
        ...
        if ($resultado) {
            ...
            if ($primeiroNaFila) {
                ...
                $professores = $this->dao->listarProfessoresPorModalidade($modalidade);
                $professorIdNovo = !empty($professores) ? $professores[0]['id'] : $professorId;
    ...
    public function cancelarPorProfessor($id, $professorId) {
        // Buscar informações do agendamento antes de cancelar
        $agendamento = $this->dao->buscarPorId($id);
        if (!$agendamento || $agendamento['professor_id'] != $professorId) {
            return false;
        }

        $resultado = $this->dao->cancelarAgendamentoPorProfessor($id, $professorId);
        ...
        if ($resultado) {
            ...
            if ($primeiroNaFila) {
                ...
                $professorIdNovo = !empty($professores) ? $professores[0]['id'] : $agendamento['professor_id'];
